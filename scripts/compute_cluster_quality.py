...
# Written by Kristoffer Sahlin for isONclust: https://github.com/ksahlin/isONclust
...
def compute_V_measure(clusters, classes):
    ...
    print("Nr reads clustered but unaligned (i.e., no class and excluded from V-measure): ", clustered_but_unaligned)
...
def get_cluster_information(clusters, classes):
    ...
    median_class_size = class_distribution[int(len(class_distribution) / 2)] if len(class_distribution) % 2 == 1 else (
        class_distribution[int(len(class_distribution) / 2)] + class_distribution[int(len(class_distribution) / 2) - 1]) / 2.0
    ...
    median_cluster_size = cluster_distribution[int(len(cluster_distribution) / 2)] if len(cluster_distribution) % 2 == 1 else (
        cluster_distribution[int(len(cluster_distribution) / 2)] + cluster_distribution[int(len(cluster_distribution) / 2) - 1]) / 2.0
...
def main(args):
    ...
    outfile.write("{0},{1},{2},{3},{4},{5},{6},{7}\n".format("tot_nr_reads", "unclassified", "nr_non_singleton_classes",
                                                             "singleton_classes", "upper_75_class_size", "median_class_size", "e_class_size", "n50_class_size"))
    outfile.write("{0},{1},{2},{3},{4},{5},{6},{7}\n".format(tot_nr_reads, unclassified, nr_non_singleton_classes,
                                                             singleton_classes, upper_75_class_size, median_class_size, e_class_size, n50_class_size))
    ...
    dfs = pd.DataFrame({'Statistic': ['Upper75ClsSize', 'Upper75ClassSize', 'MedianClsSize', 'MedianClassSize'], 'Value': [upper_75_cluster_size, upper_75_class_size, median_cluster_size, median_class_size]}).set_index('Statistic')
    dfs2 = pd.DataFrame({'Statistic': ['N50ClsSize', 'N50ClassSize'], 'Value': [n50_cluster_size, n50_class_size]}).set_index('Statistic')

    df_sizes = pd.read_csv(args.sizes, sep="\t")
    ...
    for p in ax.patches:
        ax.annotate("{:.2f}".format(p.get_height()), (p.get_x() + p.get_width() / 2.0, p.get_height()), ha='center')
    pages.savefig()
    plt.clf()

    # df_sizes.Size.plot(kind='hist', fontsize=7, logx=True, bins=50)
    # pages.savefig()
    # plt.clf()

    # sns.barplot(x="ClusterId", y="Size", data=df_sizes).set(xticklabels=[], xticks=[])
    # pages.savefig()
    # plt.clf()
    ...
    pages.savefig()
    plt.clf()
    # dfn2.plot(kind='bar', fontsize=7, rot=0)
    # pages.savefig()
    ax = dfs.plot(kind='bar', fontsize=7, rot=0)
    for p in ax.patches:
        ax.annotate("{:.2f}".format(p.get_height()), (p.get_x() + p.get_width() / 2.0, p.get_height() + yd), ha='center')
    pages.savefig()
    plt.clf()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--report', type=str, help='Output PDF file with report')
